import pygame as p
from modes.multiplayer.imports import piece as pie
from modes.multiplayer.imports import board as b
import sys
####################################
boardDIMENSION = 8                 #
width = height = 500               #
...
####################################
p.init()
"""• Firstly, we import pygame as p as it will manufacture, lay out piece images 
that will initially set up the chess board.
•	We then need to import the piece from the other python file piece.py which will 
be in a dictionary format already rendered in piece.py.
•	In the game file, we also have another folder named modes which holds the data 
to manufacture the different game modes when taped by the button in the main GUI 
screen. However, it hasn’t been completed.
•	We also import the board.py as b which imports the most vital class that holds 
data about each individual piece i.e. class piece_manager:
•	The sys module provides information about constants, functions and methods 
of the Python interpreter. 
•	Secondly, we starting defining key global variables that will be used 
throughout the program, we start with boardDIMENSION which simply means that 
it would be used when manufacture the board.
•	width/height is used to save the overall program screen with boxheight 
dividing the height with the board dimension to create 8 squares of equal 
dimension. The // function is used for integer division which gets integer value and not real.
•	FPS is the game frames/second with the dictionary rendered being the 
library where the pictures rendered by the RenderImage subroutine in the piece.py.
•	Finally, p.init() basically means that pygame is being initialised.
•	moveTURN holds dimension for producing which player’s turn it is."""
...
def playGame():
    ...
    while running:
        ...
        for event in p.event.get():
            ...
            if event.type == p.QUIT:
                ...
                if event.key == p.K_ESCAPE:
                    "If the button click is esc"
                    p.display.set_mode((width, height))
                    "It will set the window for the mainGUI pane"
                    running = False
                    "Will stop executing the loop"
        ...
        drawGameState(screen, gs, validate, square_selected, drawTurn)
        "Starts drawing the game board with the pieces."
...
def drawGameState(screen, gs, valid_moves, square_selected, moveTURNfont):
...
def drawTurn(screen, gs):
    "Subroutine that will manufacture whose turn it is on the multiplayerENGINE pane"
    ...
    player1 = playerFont.render("PLAYER 1", True, p.Color("black"))
    player2 = playerFont.render("PLAYER 2", True, p.Color("black"))
    ...
    screen.blit(player1, ((540, 105)))
    screen.blit(player2, ((540, 305)))
    screen.blit(white, ((590, 130)))
    screen.blit(black, ((590, 330)))
    "Drawing the squares and text on the pane"
...
def animateMove(move, screen, board, clock):
    ...
    "subtracts the two variables to check the distance of animation"
    FPSpieceMove = 15  # frames to move one square
