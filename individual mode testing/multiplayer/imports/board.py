class GameState:
    "Main class that will handle all piece functions."
    def __init__(self):
        """ "--" will represent the empty list, whereas every other word is a piece name directly"""
        self.board = [
            ["bR", "bN", "bB", "bQ", "bK", "bB", "bN", "bR"],
            ["bp", "bp", "bp", "bp", "bp", "bp", "bp", "bp"],
            ["--", "--", "--", "--", "--", "--", "--", "--"],
            ["--", "--", "--", "--", "--", "--", "--", "--"], # 2x2 array as there are 64 squares in a chess board.
            ["--", "--", "--", "--", "--", "--", "--", "--"],
            ["--", "--", "--", "--", "--", "--", "--", "--"],
            ["wp", "wp", "wp", "wp", "wp", "wp", "wp", "wp"],
            ["wR", "wN", "wB", "wQ", "wK", "wB", "wN", "wR"]]
        "directly imported from the pieces library in the engine.py file."
        self.white_to_move = True
        "After black, the right to move will be transferred to white."
        self.moveRegister = []
        'List that will keep track of all the moves.'
        self.white_king_location = (7, 4)
        self.black_king_location = (0, 4)
        self.checkmate = False
        'Flag variable that is set to see when the players are in checkmate.'
        self.stalemate = False
        'Flag variable that is set to see when the players are in stalemate.'
        self.in_check = False
        'Flat variable that is set to see when the players are in check.'
        self.pins = []
        self.checks = []
    ...
    def makeMove(self, move):
        "Subroutine that takes the parameter move that allows us to make the move."
        self.board[move.start_row][move.start_col] = "--"
        self.board[move.end_row][move.end_col] = move.piece_moved
        self.moveRegister.append(move)
        "log the move so we can undo it later"
        ...
        "Code that will trigger once the pieces make it to the start of the other players squares."
        if move.is_pawn_promotion:
            self.board[move.end_row][move.end_col] = move.piece_moved[0] + "Q"

        "Code that will check for en-passant moves."
    ...
    "Subroutine that is the opposite of makeMove."
    def undoMove(self):
        if len(self.moveRegister) != 0:
            "make sure that there is a move to undo"
            move = self.moveRegister.pop()
    ...
    def updateCastleRights(self, move):
        "Updates the castle rights that is given by the moves."
    ...
    # Subroutine that handles move validation.
    def moveValidation(self):
        ...
        if self.in_check:
            if len(self.checks) == 1:
                ...
                valid_squares = []
                "squares that pieces can move to saved in a list, will be passed to other subroutines"
    ...
    def inCheck(self):
        "Subroutine that will check whether if a player is in check."
        if self.white_to_move:
    ...
    def getAllPossibleMoves(self):
        "Subroutine that checks what moves have taken place and will thereby trigger the appropriate piece subroutine."
        moves = []
    ...
    def checkForPinsAndChecks(self):
        pins = []
        "squares pinned and the direction its pinned from"
        checks = []
    ...
    def getPawnMoves(self, row, col, moves):
        "Subroutine that will be trigger whenever the pawn is moved, it will be saved in a list moves then."
        piece_pinned = False
        pin_direction = ()
    ...
    def getRookMoves(self, row, col, moves):
        "Subroutine that will handle all the rook moves. It will be triggered whenever rook is moved."
        piece_pinned = False
        pin_direction = ()
        for i in range(len(self.pins) - 1, -1, -1):
            if self.pins[i][0] == row and self.pins[i][1] == col:
    ...
    def getKnightMoves(self, row, col, moves):
        "Subroutine for knight pattern moving. It will trigger once the knight has moved."
        piece_pinned = False
        for i in range(len(self.pins) - 1, -1, -1):
            if self.pins[i][0] == row and self.pins[i][1] == col:
                piece_pinned = True
    ...
    def getBishopMoves(self, row, col, moves):
        "Subroutine that will trigger whenever the bishop moves."
        piece_pinned = False
        pin_direction = ()
        for i in range(len(self.pins) - 1, -1, -1):
            if self.pins[i][0] == row and self.pins[i][1] == col:
    ...
    def getQueenMoves(self, row, col, moves):
        "A queen is basically a rook as well as a bishop. It will call the bishop and rook subroutine for minimal coding."
        self.getBishopMoves(row, col, moves)
        self.getRookMoves(row, col, moves)


    def getKingMoves(self, row, col, moves):
        "King can basically move 1 direction at a time. It will be activated whenever someone moves the king."
    ...
    def getCastleMoves(self, row, col, moves):
        "Subroutine that will take care of all the castling rights."
    ...
    def getKingsideCastleMoves(self, row, col, moves):
        "Subroutine that will take care of all the castling moves with the king."
    ...
    def getQueensideCastleMoves(self, row, col, moves):
        "Subroutine that will take care of all the castling moves with the queen."
...
class CastleRights:
    "Class where certain castle rights are defined."
...
class Move:
    "Class that will control the movement of the pieces."
    ...
    # Function that overrides the equals method.
    ...
    def getChessNotation(self):
        "Subroutine that prints the chess notation on the moveRegister_movelog."
    ...
    def getRankFile(self, row, col):
        "Will print the piece rank file."
